...
// This enables polling:
// https://en.wikipedia.org/wiki/Polling_(computer_science) I use polling for:
//      • Undo/Redo functionality
//      • Updating the UI when parameter changes come from the host
//
// The other option is to use listeners, which have the advantage of being more
// efficient However, 90% of the time, in audio code, I prefer polling parameter
// listeners CAN get called by the host from the audio thread, which leads to
// issues with thread safety and realtime safety
//==============================================================================
...
private:
  ...
  std::unique_ptr<ParameterSlider> fb_slider;
